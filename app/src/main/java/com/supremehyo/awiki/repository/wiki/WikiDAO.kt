...
interface WikiDAO {
    ...
    @Query("SELECT * FROM tb_wiki where title LIKE '%' || :title ||'%'")
    fun getListWikiBySearch(title: String): List<WikiContract>

    @Query("SELECT * FROM tb_wiki where id =:id")
    fun randomGetOneWiki(id: Long): WikiContract
